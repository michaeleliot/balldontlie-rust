...
pub async fn get_games(query_params: GamesQueryParams) -> Result<Vec<Game>, Box<dyn std::error::Error>> {
  ...
  query_params_list.append(&mut format_numbers_query_param_array("team_ids".to_string(), &query_params.team_ids));
...
pub async fn get_game(game_id: u32) -> Result<Game, Box<dyn std::error::Error>> {
    let request_url = format!("https://www.balldontlie.io/api/v1/games/{game_id}", game_id = game_id);
