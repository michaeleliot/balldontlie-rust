...
#[tokio::main]
pub async fn get_stats(query_params: StatsQueryParams) -> Result<Vec<Stat>, Box<dyn std::error::Error>> {
  ...
  if query_params.end_date.is_some() {
    query_params_list.push(("end_date".to_string(), query_params.end_date.unwrap().to_string()))
  }

  query_params_list.append(&mut format_numbers_query_param_array("player_ids".to_string(), &query_params.player_ids));
  query_params_list.append(&mut format_numbers_query_param_array("game_ids".to_string(), &query_params.game_ids));
