use crate::basketball_types::{SeasonAverages, NoMetaListReturnValue};
...
#[tokio::main]
pub async fn get_season_averages(query_params: SeasonAverageQueryParams) -> Result<Vec<SeasonAverages>, Box<dyn std::error::Error>> {
  let mut query_params_list = vec![
    ("season".to_string(), query_params.season.to_string()),
  ];
